...
// ----------------------------------------------------------------
// ==========================================
// 🔐 M-PESA TOKEN GENERATOR (ENVIRONMENT READY)
// ==========================================
async function getMpesaToken() {
    // Pull credentials securely from Railway Environment Variables
    const consumer_key = process.env.MPESA_CONSUMER_KEY;
    const consumer_secret = process.env.MPESA_CONSUMER_SECRET;

    // Fail-safe validation to protect API calls
...
app.post('/api/v1/payment/dual-stk', async (req, res) => {
    ...
    // 🔐 STEP 3: PREPARE MPESA CONFIG (PRODUCTION & RAILWAY READY)
try {
...
const callbackUrl = `${process.env.APP_URL}/api/v1/payment/callback`;
   ...
   // 🧪 THE COCKTAIL FIX: Define every single alias so the code below CANNOT fail
  const appUrl = process.env.CALLBACK_URL; 
  const app_url = process.env.CALLBACK_URL 
