...
const authenticateAdmin = (req, res, next) => {
    // Check for "x-master-key" in headers OR "key" in URL
    const key = req.headers['x-master-key'] || req.query.key;
    
    // ⚠️ REPLACE 'admin123' WITH A STRONG KEY OR process.env.MASTER_KEY
    if (key !== 'Zoey1957jepkemoi#' && key !== process.env.MASTER_KEY) {
